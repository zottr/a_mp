...
function Orders() {
  ...
  const [fetchedOrdersLength, setFetchedOrdersLength] = useState(0);
  ...
  const { loading, error, data, fetchMore } = useQuery(GET_ORDERS_LIST, {
    ...
    onCompleted: (fetchedData) => {
      ...
      setFetchedOrdersLength((prev) => prev + fetchedData.orders.items.length);
      setHasMore(
  ...
  const loadMore = () => {
    ...
    fetchMore({
      ...
      .then(({ data: fetchedData }) => {
        ...
        setFetchedOrdersLength(
          (prev) => prev + fetchedData.orders.items.length
        );
        setHasMore(
          fetchedData.orders.items.length + fetchedOrdersLength <
          ...
          >
            ...
            >
              {data?.orders?.items.map((order, index) => (
                <div
                  ref={index === fetchedOrdersLength - 1 ? lastOrderRef : null}
                  key={order.id}
                >
                  <OrderItem
                    order={order}
