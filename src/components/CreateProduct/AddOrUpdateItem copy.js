...
const AddOrUpdateItem = ({
  ...
  const [initialAssets2, setInitialAssets2] = useState([]);
  ...
  const [fetchProduct, { loading, error, data }] = useLazyQuery(
    GET_PRODUCT_TO_EDIT,
    {
      ...
      onCompleted: (fetchedData) => {
        ...
        setInitialAssets2(updatedAllImages);
        setExistingAssets(updatedAllImages);
  ...
  const handleRemoveImage = (index, isNew, closeImageMenu) => {
    ...
    if (isNew) {
      ...
      setProduct((prev) => {
        ...
        let updatedPreviewImages = [...prev.previewImages];
        updatedImages = updatedImages.filter((_, i) => i !== index);
        updatedPreviewImages = updatedPreviewImages.filter(
          (_, i) => i !== index
        );
        return {
          ...prev,
  ...
  const handleSelectCoverImage = (index, isNew, closeImageMenu) => {
    closeImageMenu();
  ...
  useEffect(() => {
    if (productToEditId != null && productToEdit2 != null) {
      ...
      if (initialAssets2) {
        const index = initialAssets2?.length ? initialAssets2.length - 1 : -1;
        setMainImageIndex(`existing-${index}`);
      }
    }
  }, [productToEdit2, initialAssets2, productToEditId]);
